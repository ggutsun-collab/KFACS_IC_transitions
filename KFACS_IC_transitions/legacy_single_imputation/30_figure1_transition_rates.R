...
## Figure 1 (MAIN) — Adjusted state-transition rates by intrinsic-capacity tertile
##
...
FIG1_SCRIPT_VERSION <- "v260811 (모든 칸 표시)"
...
msg(sprintf("Figure 1: N=%d (W1), person-waves=%d", length(unique(d$id)), nrow(d)))

collect <- function(state_var, sys_label) {
  iv <- build_intervals(d, state_var, "rolling")
  if (is.null(iv)) return(NULL)
  REF <- make_ref(iv)
  msg(sprintf("[Fig1] %s 기준 프로파일: %s", sys_label,
              paste(sprintf("%s=%s", names(REF),
                            vapply(REF, function(z) if (is.numeric(z)) sprintf("%.2f", z)
                                   else as.character(z), "")), collapse = ", ")))
  tl <- default_transitions(state_var, iv)
  r <- do.call(rbind, lapply(tl, function(tr)
    adjusted_rates_by_tertile(iv, tr[1], tr[2], ref = REF)))
  p <- strsplit(as.character(r$transition), " -> ", fixed = TRUE)
  r$from <- vapply(p, `[`, character(1), 1)
  r$to   <- vapply(p, `[`, character(1), 2)
  r$system <- sys_label
  r
}
RATE <- rbind(collect("state",        "a  ADL disability"),
              collect("frailty_3cat", "b  Frailty phenotype"))

## 95% CI (Poisson 사건수 기반)
RATE$se_log <- ifelse(RATE$events > 0, 1 / sqrt(RATE$events), NA_real_)
...
###############################################################################
RATE$cls <- "ok"
RATE$cls[RATE$events > 0 & is.na(RATE$adj_rate)] <- "nonest"
RATE$cls[!is.na(RATE$adj_rate) & (RATE$events < MIN_EV | RATE$pyears < MIN_PY)] <- "sparse"
RATE$cls[RATE$events == 0 | is.na(RATE$events)] <- "zero"
...
save_vals(RATE[, c("system","transition","from","to","tertile","events","pyears",
                   "crude_rate","adj_rate","lo","hi","RR","cls","rule3","yval")],
          "F1_transition_rates.csv", FIG_DIR)
...
CLS_LEV <- c("ok", "sparse", "zero", "nonest")
...
CLS_SHP <- c(ok = 21, sparse = 21, zero = 6, nonest = 4)
...
CLS_LTY <- c(ok = "solid", sparse = "22", zero = "blank", nonest = "solid")
...
mk <- function(sys, leg = FALSE) {
  ...
  p <- ggplot(x, aes(to, yval, colour = tertile, group = tertile)) +
    ...
    scale_colour_manual(values = PAL_TERTILE_LAB, name = "IC tertile (wave 1)", drop = FALSE) +
...
CAP <- paste0(
  ...
  "; standard errors clustered by participant. Intrinsic-capacity tertiles were defined once ",
  "from the wave-1 distribution and applied to the capacity measured at the start of each ",
  "interval. Rates are predicted at a single reference profile common to every transition, so ",
  ...
  "Filled circles with solid intervals are cells with at least ", MIN_EV, " events and at least ",
  MIN_PY, " person-years. ",
...
if (all(c("title", "caption") %in% .af)) {
  ...
  writeLines(c("Figure 1 | Adjusted state-transition rates by intrinsic-capacity tertile", "", CAP),
             file.path(FIG_DIR, "Figure1_TransitionRates_caption.txt"), useBytes = TRUE)
  message("[Fig1] 캡션을 직접 저장했습니다.")
}
save_na(fig, "Figure1_TransitionRates", width_mm = NA_WIDTH$double,
...
cat("============================================================\n")
cat("\n=== Figure 1 완료 ===  근거수치:", file.path(FIG_DIR, "F1_transition_rates.csv"), "\n")
