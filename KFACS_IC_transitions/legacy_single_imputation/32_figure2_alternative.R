...
kf_init()
F_PRED <- file.path(FIG_DIR, "F2_dynamic_predictions.csv")
F_OBS  <- file.path(FIG_DIR, "F2_observed_IC.csv")
if (!file.exists(F_PRED))
  stop("F2_dynamic_predictions.csv 가 없습니다.\n  -> 31_figure2_dynamic_prediction.R 을 먼저 한 번 실행하십시오.\n  찾은 곳: ", F_PRED)
...
save_vals(cbind(rbind(FOR, EQ), n_intervals = N_INT, n_deaths = N_DTH),
          "F2_current_vs_change.csv", FIG_DIR)
...
CAP <- paste0(
  ...
  N_DTH, " deaths), adjusted for origin state and for ", adj_phrase(),
...
save_na(fig, "Figure2_JM_dynamic_predictions", width_mm = NA_WIDTH$double,
        height_mm = 118, dir = FIG_DIR)

cat("\n=== Figure 2 (미팅 반영판) 완료 ===\n")
cat("  본문 그림 : Figure2_JM_dynamic_predictions.pdf / .tiff\n")
cat("  각주      : Figure2_JM_dynamic_predictions_caption.txt\n")
