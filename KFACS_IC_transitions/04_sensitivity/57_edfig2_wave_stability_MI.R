...
## 3. 자료와 구간
###############################################################################
## ── v260905 ED 판: MI01 완성 데이터 + within-sex 척도 (14_ 와 동일 정의); sex 는 표준화에 흡수되어 보정군에서 제외
...
CFG$ADJ <- setdiff(CFG$ADJ, "sex_f")
msg("[ED 판] MI01 + within-sex 척도, 보정군 = ", paste(CFG$ADJ, collapse = "+"))
...
save_table(TAB, "ST_WaveStability_MI",
...
save_vals(PER, "ST_WaveStability_MI_values.csv", FIG_DIR)
...
if (nrow(G)) {
  ...
  save_na(P_ALL, "ExtendedData_Fig2_WaveStability_MI", width_mm = NA_WIDTH$double,
